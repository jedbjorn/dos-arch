...
| `dr_db` | `sqlite_master` walk (`type IN (''table'',''view'')`) + curated `_DB_PURPOSE_ENTRIES`; `kind` column distinguishes tables from views |
...
| a substrate table or view (DB migration) | add/update its row in `_DB_PURPOSE_ENTRIES` in `dr_sync.py` |
