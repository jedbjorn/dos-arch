...
# broker by Docker DNS name (http://dos-broker:8788). Secrets are read at run
# time from ~/.config/dos-arch/.env via --env-file — never baked into the
...
[[ -r "${ENV_FILE}" ]] || {
  ...
  echo "       Create it:  mkdir -p ~/.config/dos-arch && cp .env.example ${ENV_FILE}" >&2
  exit 1
}
